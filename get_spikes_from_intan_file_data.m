function [waveforms,times,digin2,t_dig,sample_rate] = get_spikes_from_intan_file_data(stdout,file,path)
% function [waveforms,times,digin2,t_dig,sample_rate] = get_spikes_from_intan_file_data(stdout,file,path)
%
% SAMPLE USAGE:  
% [waveforms,times,digin2,t_dig,sample_rate] =
% get_spikes_from_intan_file_data;


% set stdout threshold for finding spikes
if nargin < 1
    stdout = 3;
end
...
[file, path, filterindex] = ...
    uigetfile('*.rhd', 'Select an RHD2000 Data File', 'MultiSelect', 'on','C:\data\sample.mat');
end

% get differnt variable type based on number of files
if iscell(file)
...
% create variables to collect data across all files
waveforms = cell(64,1);
times = cell(64,1);
allamptime = [];
alldigdata = [];
...
%                     fprintf(1, '%d%% done...\n', percent_done);
    ...
    n = 5; % order of filter/2
    lowf = 300;
    highf = 7E3;
    disp('began filtering')
    tic
    data = butterbandpass(amplifier_data, n, lowf, highf, sample_rate );
    toc
    disp('finished filtering')
    
    [ thresholds ] = getinteractivethresholds( data, sample_rate ,3, 0 );
    for gq = 1:size(amplifier_data,1)
    [thisepochtimes, thisepochwaveforms] = Get_Spikes(data(gq,:),t_amplifier,thresholds(gq),sample_rate);
        waveforms{gq} = [waveforms{gq}; thisepochwaveforms];
        times{gq} = [times{gq}; thisepochtimes];
    end
    disp('finished gettnig spikes')

%     allampdata = [allampdata amplifier_data];
    allamptime = [allamptime t_amplifier];
    alldigdata = [alldigdata board_dig_in_data];
    alldigtime = [alldigtime t_dig];
    
    fprintf('File %d of %d processed\n',f,lengthfile)
end % for f = 1:lengthfile

% amplifier_data = allampdata;
t_amplifier = allamptime;
board_dig_in_data = alldigdata;
t_dig = alldigtime;

% move_to_base_workspace(amplifier_data);
move_to_base_workspace(t_amplifier);
move_to_base_workspace(board_dig_in_data);
digin2 = board_dig_in_data(2,:);
